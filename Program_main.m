...
for p=1:P;
    ...
    for j=1:length(h);
        ...
        q=0;    %increase q to decrease window length
        for i=1:size(GW,2);
            X_i_j_p=[ones(R+(p-1)-h(j)-p+1-q,1) GW(p+q:R+(p-1)-h(j),i)];
            results_i_j_p=ols(r_h((1+q+p):R+p-h(j),j),X_i_j_p);
    ...
    sumweights = zeros(length(h));
    weightedAV = zeros(P,length(h));
    for j=1:length(h);
    ...
    MSFE_PR_j=sum(u_PR_j.^2);
    DMSPE = sum(MSFE_PR_j);
   for i = 1:15
    sumweights(j) = sumweights(j) + ((MSFE_PR_j(i)/ DMSPE))^-1;
   end
    for i = 1:15
        weightedAV(:,j) = weightedAV(:,j) + (((MSFE_PR_j(i)/ DMSPE))^-1)*FC_PR(:,i,1);
    end
    weightedAV(:,j) = weightedAV(:,j)/sumweights(j);
    end
    
  
    R2OS_weights = zeros(1,length(h));
    for j=1:length(h);
    actual_j=r_h(R+1:end-(h(j)-1),j);
    u_PM_j=actual_j-FC_PM(1:end-(h(j)-1));
    u_PR_j=kron(ones(1,1),actual_j)-weightedAV(1:end-(h(j)-1),j);
    MSFE_PM_j=mean(u_PM_j.^2);
    MSFE_PR_j=mean(u_PR_j.^2);
    R2OS_weights(j) = 100*(1-MSFE_PR_j/MSFE_PM_j);
    end
